...
int search_records(int key, struct Record hash_table[])
{
      ...
      for(count = 1; count != LIMIT - 1; count++)
      {
            ...
            if(hash_table[position].info.employee_id == key && hash_table[position].status != DELETED)
            {
                  return position;
            }
...
void display_records(struct Record hash_table[])
{
      ...
      for(count = 0; count < LIMIT; count++)
      {
            ...
            {
                  printf("Occupied - ID: %d Name: %s Age: %d\n",hash_table[count].info.employee_id, hash_table[count].info.employee_name, hash_table[count].info.employee_age);
            }
